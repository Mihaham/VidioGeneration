...
def generate_audio_with_salut(prompt: str, retry : int = 10) -> str:
            ...
            Chat(messages=[
                Messages(
                    ...
                    content="""
                            ...
                            - Заголовки
                            - Разделители (---, ===)
                            3. Строго 10-15 предложений (~400 символов)
    ...
    if not (30 < AudioFileClip(str(audio_path)).duration < 90) and retry > 0:
        logger.warning(f"AUDIO IS NOT IN CORRECT DURATION : {AudioFileClip(str(audio_path)).duration}")
        return generate_audio_with_salut(prompt, retry - 1)
